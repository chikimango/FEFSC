...
/**
 * @author Melanie  10/23/2016
 * JavaFX Fire Emblem Fates Automatic
 * Support Coder
 * v1.0
 */
...
/*TODO: Fix this mess ↓
        //new window to show how support would look in game (or close to it)
        Stage stage3 = new Stage();
        stage3.setTitle("Support Viewer");
        final GridPane supportViewer = new GridPane();
        supportViewer.setAlignment(Pos.CENTER);
        supportViewer.setHgap(20);
        supportViewer.setVgap(20);
        supportViewer.setPadding(new Insets(15, 15, 15, 15));
        final Scene scene3 = new Scene(supportViewer, 540, 324);
        scene3.getStylesheets().add(this.getClass().getResource("font.css").toExternalForm());

        Image bg = new Image(getClass().getResourceAsStream("/portraits/SupportBG.png"));
        ImageView bgView = new ImageView(bg);
        bgView.setPreserveRatio(true);
        bgView.setFitHeight(bg.getHeight() * 1.35);
        bgView.setFitWidth(bg.getWidth() * 1.35);

        Image textBox = new Image(getClass().getResourceAsStream("/portraits/TextBox.png"));
        ImageView textBoxView = new ImageView(textBox);
        textBoxView.setPreserveRatio(true);
        textBoxView.setFitHeight(textBox.getHeight() * 1.35);
        textBoxView.setFitWidth(textBox.getWidth() * 1.35);
        textBoxView.setTranslateX(10 * 1.35);
        textBoxView.setTranslateY(92 * 1.35);

        Image nameBox = new Image(getClass().getResourceAsStream("/portraits/NameBox.png"));
        ImageView nameBoxView = new ImageView(nameBox);
        nameBoxView.setPreserveRatio(true);
        nameBoxView.setFitHeight(nameBox.getHeight() * 1.35);
        nameBoxView.setFitWidth(nameBox.getWidth() * 1.35);
        nameBoxView.setTranslateX(8 * 1.35);
        nameBoxView.setTranslateY(59 * 1.35);
        //nameBoxView.setVisible(false);

        Image keyPress = new Image(getClass().getResourceAsStream("/portraits/KeyPress.png"));
        ImageView keyPressView = new ImageView(keyPress);
        keyPressView.setPreserveRatio(true);
        keyPressView.setFitHeight(keyPress.getHeight() * 1.35);
        keyPressView.setFitWidth(keyPress.getWidth() * 1.35);
        keyPressView.setTranslateX(365 * 1.35);
        keyPressView.setTranslateY(100 * 1.35);
        keyPressView.setVisible(false);


        final TextArea dialogueView = new TextArea("It takes place between Sakura and Elise.aaa");
        dialogueView.setWrapText(true);
        dialogueView.setMaxSize(320,44);//422, 60);
        dialogueView.getStyleClass().add("textArea2");
        dialogueView.setScaleX(1.35);
        dialogueView.setScaleY(1.35);
        dialogueView.setTranslateX(94);
        dialogueView.setTranslateY(125);
        dialogueView.setEditable(false);

        final TextArea speakerView = new TextArea("    Sakura");
        speakerView.setWrapText(true);
        speakerView.setMaxSize(95,24);//422, 60);
        speakerView.getStyleClass().add("textArea3");
        speakerView.setScaleX(1.35);
        speakerView.setScaleY(1.35);
        speakerView.setTranslateX(32);
        speakerView.setTranslateY(100);
        speakerView.setEditable(false);

        supportViewer.getChildren().addAll(bgView, textBoxView, nameBoxView, dialogueView, speakerView);
        ...
        /*supportViewerBtn.setOnAction(new EventHandler<ActionEvent>() {
            @Override
            public void handle(ActionEvent event) {
                //String support = supportArea.getText();
                //int slideNum = 1 + StringUtils.countMatches(support, "$k$p") + StringUtils.countMatches(support, "$k\\n");
                /*ImageView[] slides = new ImageView[slideNum];
                String[] dialogue = new String[slideNum];

                String speaker[] = new String[slideNum];

                while (support.contains("$k$p") || support.contains ("$k\\n")) {
                    for (int i = 0; i < slideNum; i++) {

                        //find dialogue to display with $Wa and $k, or $Wa$E and |
                        dialogue[i] = support.substring(support.indexOf("$Wa"), support.indexOf("$k"));
                        if (dialogue[i].contains("$E")) {
                            dialogue[i] = dialogue[i].substring(dialogue[i].indexOf("|") + 1);
                        }

                        speaker[i] = support.substring(support.indexOf("$Ws", support.indexOf("|")));

                        //find placement of character with $Wm and |7 or |3
                        if (support.substring(support.indexOf("$k\\n"), support.indexOf("$k")).contains("|3")) {
                            String chara1 = support.substring(support.indexOf("$Wm"), support.indexOf("|3"));
                        }
                        if (support.substring(support.indexOf("$k\\n"), support.indexOf("$k")).contains("|7")) {
                            String chara2 = support.substring(support.indexOf("$Wm"), support.indexOf("|7"));
                        }

                        //find portrait to display with $Wa and |
                        String emotion = "";
                        String emotion2 = "";

                        if (support.substring(StringUtils.indexOf(support, "$k"), StringUtils.indexOf
                                (support, "$k", StringUtils.indexOf(support, "$k")+2)).contains("$Wa") &&
                                !support.substring(StringUtils.indexOf(support, "$k"), StringUtils.indexOf
                                        (support, "$k", StringUtils.indexOf(support, "$k")+2)).contains("$E")) {
                            emotion = "通常";
                        }
                        if (support.substring(StringUtils.indexOf(support, "$k"), StringUtils.indexOf
                                (support, "$k", StringUtils.indexOf(support, "$k")+2)).contains("$Wa$E")) {
                            emotion = support.substring(support.indexOf("$Wa$E")+5, support.indexOf(","));
                        }
                        if (!support.substring(StringUtils.indexOf(support, "$k"), StringUtils.indexOf
                                (support, "$k", StringUtils.indexOf(support, "$k")+2)).contains(",|")) {
                            emotion2 = support.substring(StringUtils.indexOf(support, ",")+1, StringUtils.indexOf
                                    (support, "|", StringUtils.indexOf(support, "$Wa$E")));
                        }
                        ...
                        if (support.contains$a$t) {
                            support = StringUtils.remove(support , support.substring(StringUtils.indexOf(support, "$a$t"), StringUtils.indexOf
                            (support, "$k"));
                        }
                        else {
                            support = StringUtils.remove(support , support.substring(StringUtils.indexOf(support, "$k"), StringUtils.indexOf
                                (support, "$k", StringUtils.indexOf(support, "$k")+2)));
                        }
                    }
                }


                String support = "$a$t1$Wmアクア|7$w0|$Wsヒノカ|$Wa$E苦,|Azura! Good. I found you!$k\\n$Wsアクア|$Wa$E苦,|Are you all right? You seem a little out of\\nbreath.";
                String chara1 = "";
                String chara2 = "";
                ...
                //find placement of character with $Wm and |7 or |3
                /*if (support.substring(StringUtils.indexOf(support, "$a$t"), StringUtils.indexOf(support, "$k")).contains("|3")
                        || support.substring(StringUtils.indexOf(support, "$k"), StringUtils.indexOf
                        (support, "$k", StringUtils.indexOf(support, "$k")+2)).contains("|3"))
                {
                ...
                if (support.contains("$a$t")) {
                    ...
                    if((support.substring(StringUtils.indexOf(support, "$a$t"), StringUtils.indexOf(support, "$k")).contains("|3"))
                            && !support.substring(StringUtils.indexOf(support, "$a$t"), StringUtils.indexOf(support, "$k")).contains("|7"))
                    {
                        chara1 = support.substring(support.indexOf("$Wm")+3, support.indexOf("|3"));
                    }

                    if((support.substring(StringUtils.indexOf(support, "$a$t"), StringUtils.indexOf(support, "$k")).contains("|7"))
                            && !support.substring(StringUtils.indexOf(support, "$a$t"), StringUtils.indexOf(support, "$k")).contains("|3"))
                    {
                        chara2 = support.substring(support.indexOf("$Wm")+3, support.indexOf("|7"));
                    }

                    if((support.substring(StringUtils.indexOf(support, "$a$t"), StringUtils.indexOf(support, "$k")).contains("|7"))
                            && support.substring(StringUtils.indexOf(support, "$a$t"), StringUtils.indexOf(support, "$k")).contains("|3"))
                    {
                        chara1 = support.substring(StringUtils.indexOf(support, "$Wm", StringUtils.indexOf(support, "$Wm")+3)+3, StringUtils.indexOf
                                (support, "|3"));
                        chara2 = support.substring(support.indexOf("$Wm")+3, support.indexOf("|7"));
                    }
                }
                    ...
                    if(support.substring(StringUtils.indexOf(support, "$k"), StringUtils.indexOf
                            (support, "$k", StringUtils.indexOf(support, "$k")+2)).contains("|3")
                            && !support.substring(StringUtils.indexOf(support, "$k"), StringUtils.indexOf
                            (support, "$k", StringUtils.indexOf(support, "$k")+2)).contains("|7"))
                    {
                        chara1 = support.substring(support.indexOf("$Wm")+3, support.indexOf("|3"));
                    }
                ...
                /*substring(StringUtils.indexOf(support, "$a$t"), StringUtils.indexOf(support, "$k")).contains("|7")
                        || support.substring(StringUtils.indexOf(support, "$k"), StringUtils.indexOf
                        (support, "$k", StringUtils.indexOf(support, "$k")+2)).contains("|7")
                        && (!support.substring(StringUtils.indexOf(support, "$k"), StringUtils.indexOf
                        (support, "$k", StringUtils.indexOf(support, "$k")+2)).contains("|3")
                ...
                //find dialogue
                //String dialogue = support.substring(support.indexOf("$Wa"), support.indexOf("$k"));
                ...
                //supportArea.appendText("" + slideNum);

                //find who's speaking with $Ws and |
                //find placement of character with $Wm and |7 or |3
                //find dialogue to display with $Wa and $k, or $Wa$E and |
